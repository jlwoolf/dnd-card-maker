...
test.describe("Card Editor", () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Hide the deck panel so it doesn't overlap the bottom card menu buttons.
    // CSS display:none is deterministic across all browsers, unlike force-clicking the
    // collapse button which may not trigger React state changes reliably in WebKit.
    await page.evaluate(() => {
  ...
  test("resets colors to defaults via color settings", async ({ page }) => {
    await page.getByTestId("open-colors-btn").click();
